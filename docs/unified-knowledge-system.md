...
> 版本: v1.1 | 日期: 2026-04-03 | 状态: 设计阶段（已审查修正）
...
CREATE TABLE knowledge_entities (
  ...
  type          TEXT NOT NULL,
    -- person      人物（通常是用户自己）
    -- project     项目/作品
    -- skill       技能/技术/工具/框架（合并 skill+tool，避免边界模糊导致跨文件对齐失败）
    -- company     公司/组织
    -- school      学校
    -- award       荣誉/奖项
    -- publication 论文/出版物
...
**查询效率说明：**
- `knowledgeEntities.fileIds` 是 JSON 数组存储，查询"某文件涉及哪些实体"需要遍历
- 当前阶段（SQLite, 实体数 < 200）不是瓶颈，遍历 + JSON.parse 在 < 1ms 内完成
- 更高效的反向查询：通过 `knowledgeFiles.entityIds` 字段直接获取该文件的实体 ID 列表
- 后续优化方向：如果实体量级增长到 1000+，可加一张 `entity_file_links(entityId, fileId)` 关联表做正规化
...
- person: 人物（文件作者、提到的人）
- project: 项目/作品/产品
- skill: 技能/编程语言/技术能力/工具/框架/平台（统一归为 skill，不区分 tool）
- company: 公司/组织/团队
- school: 学校/教育机构
- award: 荣誉/奖项/证书
...
```markdown
给定以下实体列表及其出现的文件，推断它们之间的关系。

## 实体列表（含文件共现信息）
${entities.map(e => {
  const files = e.fileIds.map(fid => fileNameMap[fid]).join(', ');
  return `- [${e.type}] ${e.name}: ${e.description}\n  出现在: ${files}`;
}).join('\n')}

## 文件共现提示
同时出现在同一个文件中的实体，很可能有直接关系。例如：
- 某个 skill 和某个 project 都出现在同一份项目文档中 → 该项目使用了该技能
- 某个 person 和某个 company 都出现在简历中 → 该人在该公司工作过
...
## 输出格式
JSON 数组:
[{"from": "实体名", "to": "实体名", "type": "关系类型", "evidence": "关键依据（含来源文件名）", "strength": 1-3}]

## 规则
- 优先根据文件共现来推断关系
- strength: 3=同文件中明确陈述, 2=同文件中可推断, 1=跨文件弱关联
- 只输出有依据的关系，不要猜测
- 输出纯 JSON
```
...
**涉及文件:**
- `src/lib/build-agents.ts` — `BuildConversationContext` 接口 + 6 个 Agent prompt 函数
- `src/app/api/chat-build/route.ts` — 调用方

**接口变更：**

```typescript
// build-agents.ts: BuildConversationContext 改动
export interface BuildConversationContext {
  // ... 保留字段 ...
  knowledgeContext: string;
  knowledgeSummary: string;
  entityContext?: string;          // 新增: 实体关系图 markdown
  // 移除: knowledgeGroupIndex    // 废弃 Legacy groups 索引
  // ... 其余不变 ...
}
```

**6 个 Agent prompt 函数都需要注入 entityContext：**
- `runOrchestratorAgent` — 注入 summary
- `runIdeationAgent` — 注入完整关系图 + 原文
- `runPlanningAgent` — 注入完整关系图 + fileRouting
- `runExecutionAgent` — 注入 summary
- `runDesignAgent` — 注入完整关系图
- `runCodeAgent` — 注入完整关系图 + 原文

**chat-build/route.ts 改造：**
...
**涉及文件:**
- `src/app/api/compile-spec/route.ts` — 后端，改数据源
- `src/app/create/page.tsx` — 前端，`compileSiteSpec()` 函数改参数

**前端改造（create/page.tsx）：**

```typescript
// ---- 改前 ----
// compileSiteSpec() 内部传 knowledge: items.filter(selected) (Legacy items)

// ---- 改后 ----
// compileSiteSpec() 改为传 knowledgeBaseIds
spec = await compileSiteSpec({
  siteType, theme, layout, customTheme,
  conversationSummary,
  techStackHints, assetIdeas,
  knowledgeBaseIds: selectedBaseIds,  // 新增，替代 knowledge[]
  // 移除: knowledge: items.filter(selected)
});
```

**后端改造（compile-spec/route.ts）：**
...
// ---- 改后 ----
// 从 knowledgeBaseIds 加载实体上下文
const { knowledgeBaseIds, intent, skillIds } = await req.json();
const session = await auth();
const entityCtx = await buildEntityContextForBases(session.user.id, knowledgeBaseIds);
const kbIndexContent = await loadKBIndexes(session.user.id);
...
#### 阶段 C: WorkspaceData 增强（新增）

> **重要说明：** 当前生成管线全程使用 `WorkspaceData`（定义在 `types.ts`），而非 `ContentModel`（定义在 `content-model.ts`）。虽然 `ContentModel` 设计更好且有 `buildContentModelFromKnowledge()` 函数，但它**从未被生成管线调用**。
>
> 因此实体增强函数应针对 `WorkspaceData`，而非 `ContentModel`。后续可考虑将管线整体迁移到 ContentModel，但不在本方案范围内。

```typescript
// 替代当前未接入的 resolveRelations() + enrichModelWithRelations()
import { enrichWorkspaceDataFromEntities } from "./entity-context";

// 在 WorkspaceData 构建完成后、传给 /api/generate 之前调用
const enrichedData = enrichWorkspaceDataFromEntities(data, entityCtx.entities);
```

**增强逻辑：**

```typescript
import type { WorkspaceData, ProjectItem, TimelineItem, SkillGroup } from "./types";

export function enrichWorkspaceDataFromEntities(
  data: WorkspaceData,
  entities: EntityWithRelations[],
): WorkspaceData {
  const result = { ...data };
  ...
  if (result.projects) {
    result.projects = result.projects.map(project => {
      ...
      const enrichedTags = [...new Set([...(project.tags || []), ...techSkills])];

      return { ...project, tags: enrichedTags };
    });
  }

  // 2. 时间线自动关联项目 (from contributed_to relations)
  if (result.timeline) {
    result.timeline = result.timeline.map(item => {
      // 从 title 中提取公司名 (格式通常是 "公司 · 职位")
      const company = item.title.split("·")[0]?.trim();
      const companyEntity = findMatchingEntity(entities, "company", company);
      if (!companyEntity) return item;

      const projects = companyEntity.relations
        .filter(r => r.relationType === "contributed_to")
        .map(r => r.targetEntity.name);
      if (projects.length > 0 && !item.desc.includes(projects[0])) {
  ...
  // 3. 补充 chatbotContext 中的关系信息
  if (result.chatbotContext) {
    const relationSummary = entities
      .filter(e => e.relations.length > 0)
      .map(e => {
        const rels = e.relations.map(r => `${r.relationType} → ${r.targetEntity.name}`).join(", ");
        return `${e.name}: ${rels}`;
      }).join("\n");
    if (relationSummary) {
      result.chatbotContext += `\n\n## 实体关系\n${relationSummary}`;
    }
  }
...
> **后续优化方向：** 将生成管线从 `WorkspaceData` 迁移到 `ContentModel`。`ContentModel` 有 `knowledgeIds` 字段、更结构化的数据模型，天然适合实体关联。但这是更大的重构，建议在本方案完成并验证后再做。
```
...
src/lib/
  ...
  entity-resolver.ts     — 跨文件实体对齐 (纯规则, skill/tool 合并为 skill)
  entity-relations.ts    — 关系推断 (AI 调用, 含文件共现上下文)
  entity-context.ts      — 构建管线消费层 (buildEntityContext + enrichWorkspaceDataFromEntities)
```
...
```
/api/kb/[baseId]/files/[fileId]  DELETE  增加 cleanupEntitiesForFile() 级联清理
```
...
```
build-agents.ts         — BuildConversationContext 接口加 entityContext 字段
                          6 个 Agent prompt 函数注入 entityContext
                          移除 knowledgeGroupIndex
chat-build/route.ts     — 加载 entityContext, 传给 runBuildConversation
compile-spec/route.ts   — 改为接收 knowledgeBaseIds, 用 entityContext 替代 Legacy items
build-runtime.ts        — enrichWorkspaceDataFromEntities() + fileRouting
create/page.tsx         — compileSiteSpec() 参数改为传 knowledgeBaseIds
```

> **注意:** 增强函数针对 `WorkspaceData`（当前管线使用的类型），不是 `ContentModel`。
> 后续可考虑整体迁移到 ContentModel，但不在本阶段。
...
当用户删除一个 knowledgeFile 时，需要级联清理实体数据：

```typescript
// 在 DELETE /api/kb/[baseId]/files/[fileId] 中添加:
async function cleanupEntitiesForFile(userId: string, fileId: string) {
  // 1. 加载所有关联该文件的实体
...
async function getKnowledgeForBuild(userId: string, baseIds: string[]) {
  // 加载 KB 原文 (始终需要)
  const kbContent = await loadKBContent(userId, baseIds);
...
| `skill` | skills | Python, React, Docker, Redis（合并了 tool 类型） |
| `company` | timeline | 字节跳动 |
| `school` | education | 清华大学 |
| `award` | awards | ACM 金牌 |
| `publication` | publications | 论文标题 |

> 注：原设计中有 `tool` 类型（工具/框架/平台），已合并到 `skill`。原因：Python/React/Docker 等在不同文件中可能被标注为 skill 或 tool，类型不一致会导致跨文件实体对齐失败。
