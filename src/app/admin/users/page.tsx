    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 pt-24 pb-12">
        ...
        {loading ? (
          ...
          <p className="text-white/30 text-sm">{zh ? "暂无用户" : "No users yet."}</p>
        ) : (
          <div className="space-y-2">
            <div className="grid grid-cols-12 gap-3 px-4 py-2 text-[10px] text-white/25 uppercase tracking-wider">
            ...
            {users.map(u => (
              <div key={u.id} className="grid grid-cols-12 gap-3 items-center px-4 py-3 rounded-xl bg-white/[0.03] border border-white/5">
                <div className="col-span-3 flex items-center gap-2">
                  <div className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-[10px] text-white/50 shrink-0">{u.name?.[0] || u.email?.[0] || "U"}</div>
                ...
                <div className="col-span-1"><span className={`text-[10px] px-1.5 py-0.5 rounded ${u.role === "admin" ? "bg-accent/20 text-accent" : "bg-white/5 text-white/30"}`}>{u.role || "user"}</span></div>
                <div className="col-span-1 text-xs text-white/40 text-center">{u.siteCount}</div>
                <div className="col-span-2 text-xs text-white/40 text-center">{u.knowledgeCount}</div>
                <div className="col-span-2 text-[10px] text-white/20">{u.createdAt ? new Date(u.createdAt).toLocaleDateString() : "—"}</div>
              </div>
            ))}
          </div>
        )}
