      ...
      <div className="relative z-10 w-full max-w-sm mx-auto px-6">
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold">
            {t("register.title")}<span className="text-accent">{t("register.titleHighlight")}</span>
          </h1>
          <p className="text-sm text-text-muted mt-2">{t("register.subtitle")}</p>
        </div>
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div>
            <label className="block text-xs text-white/40 mb-1.5">{t("register.name")}</label>
          ...
          <div>
            <label className="block text-xs text-white/40 mb-1.5">{t("register.password")}</label>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required
              className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-accent/50"
              placeholder={t("register.passwordPlaceholder")} />
          </div>
          <div>
            <label className="block text-xs text-white/40 mb-1.5">{t("register.confirmPassword")}</label>
        ...
        <p className="text-center text-sm text-white/30 mt-6">
          ...
          <Link href="/login" className="text-accent hover:text-accent/80 transition-colors">{t("register.login")}</Link>
        </p>
      </div>
    </div>
  );
