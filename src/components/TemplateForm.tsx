...
export default function TemplateForm({ initialData, onSubmit }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div>
        <label className="block text-sm font-medium mb-2">{zh ? "模板名称" : "Template Name"}</label>
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} required
          ...
          placeholder={zh ? "如：开发者作品集 Pro" : "e.g., Developer Portfolio Pro"} />
      </div>

      <div>
        <label className="block text-sm font-medium mb-2">{zh ? "描述" : "Description"}</label>
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3}
          ...
          placeholder={zh ? "描述此模板最适合什么场景..." : "Describe what this template is best for..."} />
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-2">{zh ? "站点类型" : "Site Type"}</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-2">{zh ? "默认主题" : "Default Theme"}</label>
        ...
        <div>
          <label className="block text-sm font-medium mb-2">{zh ? "默认布局" : "Default Layout"}</label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-2">{zh ? "预览图 URL" : "Preview Image URL"}</label>
          <input type="text" value={previewImage} onChange={(e) => setPreviewImage(e.target.value)}
            className="w-full px-4 py-3 rounded-xl bg-gray-100 border border-gray-200 text-sm text-gray-900 placeholder:text-gray-500 focus:outline-none focus:border-accent/50"
            placeholder="https://..." />
        </div>
        <div>
          <label className="block text-sm font-medium mb-2">{zh ? "预览 URL" : "Preview URL"}</label>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm text-gray-600">{zh ? "推荐模板" : "Featured template"}</span>
      </div>
      ...
      <div className="flex items-center gap-3 pt-4">
        <button type="submit" disabled={saving || !name}
          ...
          {saving ? (zh ? "保存中..." : "Saving...") : initialData ? (zh ? "更新模板" : "Update Template") : (zh ? "创建模板" : "Create Template")}
        </button>
