// hero/brutalist.ts
// Extracted from genBrutalistPage — large centered avatar + raw text + social icons.
// Dark coder aesthetic with big avatar, bold name, subtitle, location, and social SVGs.
import type { SectionContext, SectionVariantFn } from "../types";

export const heroBrutalist: SectionVariantFn = (ctx) => {
  const { data } = ctx;
