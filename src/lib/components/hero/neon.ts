// hero/neon.ts
// Extracted from genGlassmorphismPage — neon text + glow avatar ring in sidebar layout.
// The main-area hero section with neon-highlighted name, bio, and about tags.
// NOTE: The glassmorphism page uses a sidebar layout; the sidebar avatar card
// is in sidebar-card.ts. This variant covers the main content hero block.
import type { SectionContext, SectionVariantFn } from "../types";

export const heroNeon: SectionVariantFn = (ctx) => {
  const { data } = ctx;
