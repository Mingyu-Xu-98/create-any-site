// hero/editorial.ts
// Extracted from genBlogPage — avatar + typed lines + social icons.
// Blog-style hero with rounded avatar, welcome comment, name heading,
// tagline, bio text, and social link buttons.
import type { SectionContext, SectionVariantFn } from "../types";

export const heroEditorial: SectionVariantFn = (ctx) => {
  const { data } = ctx;
